          ...
          <div className="relative grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-white/10">
            {stats.map((s, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`px-5 py-5 sm:px-6 sm:py-6 reveal reveal-delay-${i + 1} ${visible ? "is-visible" : ""}`}
                >
                  <div className="mb-2">
                    <Glyph play={visible} />
                  </div>
                  <div className="text-section text-white tabular-nums">
                    <CountUp value={s.value} start={visible} durationMs={1300 + i * 150} />
                  </div>
                  <div className="text-label text-primary mt-1.5">{s.unit}</div>
                  <p className="mt-2 text-caption text-[hsl(30_20%_82%)] max-w-[280px]">
                    {s.body}
                  </p>
                </div>
              );

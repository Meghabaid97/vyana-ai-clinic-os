...
const DashboardBriefingHero = ({ hasRecords }: Props) => {
  ...
  return (
    <section className="px-4 sm:px-5 pb-4 lg:px-0 lg:pb-0">
      <div
        className="relative overflow-hidden rounded-[2rem] bg-primary p-6 lg:p-7 text-primary-foreground shadow-[0_12px_32px_-12px_hsl(var(--primary)/0.45)]"
      >
        {/* soft glow accent */}
        <div className="pointer-events-none absolute -right-10 -bottom-12 h-40 w-40 rounded-full bg-white/10 blur-2xl" />
        <div className="pointer-events-none absolute -left-8 -top-10 h-32 w-32 rounded-full bg-white/8 blur-2xl" />

        <div className="relative">
          <div className="flex items-center gap-2 mb-3" style={stage(0)}>
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/15 backdrop-blur px-2.5 py-1 text-[10.5px] font-semibold tracking-wider uppercase text-primary-foreground ring-1 ring-white/20">
              <Clock className="h-3 w-3" /> {t("briefingHero.eyebrow")}
            </span>
            <span className="inline-flex items-center gap-1 text-[10.5px] font-medium text-primary-foreground/85">
          ...
          <p
            className="mt-2 text-[13px] lg:text-[14px] text-primary-foreground/85 leading-snug max-w-[42ch] lg:max-w-[54ch]"
            style={stage(220)}
          >
            {t("briefingHero.body")}
          </p>

          {/* Primary CTA — white pill on coral, exactly matches v3 */}
          <button
            onClick={() => navigate("/app/briefing")}
            style={stage(340)}
            className="group mt-5 inline-flex items-center gap-3 rounded-2xl bg-white text-primary px-5 py-3 font-bold shadow-[0_4px_14px_-2px_rgba(0,0,0,0.15)] transition-all duration-200 hover:-translate-y-0.5 active:translate-y-0 active:scale-[0.98]"
          >
            <span className="inline-flex h-7 w-7 rounded-lg bg-primary/10 items-center justify-center">
          ...
          {/* Secondary actions — translucent chips on coral */}
          <div className="mt-4 grid grid-cols-2 gap-2.5">
            <button
              onClick={() => navigate("/app/share")}
              style={stage(460)}
              className="flex items-center gap-2 rounded-xl bg-white/15 backdrop-blur ring-1 ring-white/20 px-3 py-2.5 text-left hover:bg-white/25 transition-colors min-w-0"
            >

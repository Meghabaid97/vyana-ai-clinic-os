...
const DoctorVisitMode = () => {
  ...
  const generate = async () => {
    ...
    try {
      ...
      void logHealthRecordsAccess({
        ...
        where: "DoctorVisitMode.generate",
        count: (recRes.data as unknown[] | null)?.length ?? 0,
        error: (recRes as { error?: unknown }).error,
      });
      const foreign = (recRes.data as Array<{ patient_id?: string }> | null)?.filter(
  ...
  const briefingToText = (): string => {
    ...
    if (briefing.drug_interactions?.interactions?.length) {
      t += `\n🧪 *Drug Interaction Flags:*\n`;
      briefing.drug_interactions.interactions.forEach(d => {
        t += `• ${d.drugs.join(" + ")} [${d.severity}]: ${d.description}${d.recommendation ? ` → ${d.recommendation}` : ""}\n`;
      });
    }
